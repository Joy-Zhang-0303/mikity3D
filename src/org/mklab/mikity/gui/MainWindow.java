...
//import org.mklab.mikity.action.mouse.ChangeConnectModeAction;
...
public class MainWindow extends ApplicationWindow {
	...
	private Action FILE_NEW_ACTION = new FileNewAction(this);
	private Action FILE_OPEN_ACTION = new FileOpenAction(this);
	private Action ANIMATION_WINDOW_OPEN_ACTION = new AnimationWindowOpenAction(this);
	private Action FILE_SAVE_ACTION = new FileSaveAction(this);
	private Action FILE_SAVE_AS_ACTION = new FileSaveAsAction(this);
	private Action FILE_IMPORT_ACTION = new FileImportAction(this);
	private Action MODELER_OPEN_ACTION = new ModelEditorOpenAction(this);
	private Action CONFIGDIALOG_OPEN_ACTION = new ConfigDialogOpenAction(this);
	private Action FILE_EXIT_ACTION = new FileExitAction(this);
	//private Action ADD_PRIM_ACTION = new AddPrimitiveAction(this);

	private Composite comp;
	private CollisionCanceller dc = new CollisionCanceller(comp);

	private Action TOOLBAR_BOX_ACTION = new BoxToolBarAction(this,dc);
	private Action TOOLBAR_SPHERE_ACTION = new SphereToolBarAction(this,dc);
	private Action TOOLBAR_CYLINDER_ACTION = new CylinderToolBarAction(this,dc);
	private Action TOOLBAR_CONE_ACTION = new ConeToolBarAction(this,dc);
	...
	protected Control createContents(Composite composite) {
		...
		modeler = new Modeler(comp,SWT.NONE,root,dc);
		modeler.setLayoutData(new GridData(GridData.FILL_BOTH));
...
//		createMainButtonComp(comp);
	...
	public void createMainButtonComp(final Composite composite) {
		...
		newModelButton = new Button(comp, SWT.NONE);
		newModelButton.setText("モデルの新規作成");

		modelerButton = new Button(comp, SWT.NONE);
		modelerButton.setText("モデルの編集");

		configButton = new Button(comp, SWT.NONE);
		configButton.setText("設定");

		simButton = new Button(comp, SWT.NONE);
		simButton.setText("シミュレーションを見る");

		saveAsButton = new Button(comp, SWT.NONE);
		saveAsButton.setText("ファイルの別名保存");

		saveButton = new Button(comp, SWT.NONE);
		saveButton.setText("ファイルを保存して終了");

//		編集できないようにする
//		setEditable(false);

		newModelButton.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {
		...
		modelerButton.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {

			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				MODELER_OPEN_ACTION.run();
			}
		});

		configButton.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {

			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				CONFIGDIALOG_OPEN_ACTION.run();
			}
		});

		simButton.addSelectionListener(new org.eclipse.swt.events.SelectionAdapter() {

			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				ANIMATION_WINDOW_OPEN_ACTION.run();
			}
		});
...
//		manager.add(primitive);
		...
		play.add(ANIMATION_WINDOW_OPEN_ACTION);
		manager.add(play);
	...
	protected ToolBarManager createToolBarManager(int arg0) {
		...
		toolbar.add(TOOLBAR_BOX_ACTION);
		toolbar.add(TOOLBAR_SPHERE_ACTION);
		toolbar.add(TOOLBAR_CYLINDER_ACTION);
		toolbar.add(TOOLBAR_CONE_ACTION);
		toolbar.add(TOOLBAR_TRIANGLE_ACTION);
		toolbar.add(TOOLBAR_QUAD_ACTION);
		return toolbar;
	}
	...
	private final class SaveAsButtonSelectionListener extends SelectionAdapter {

		public void widgetSelected(SelectionEvent arg0) {
			FILE_SAVE_AS_ACTION.run();
		}
	}
...
//		setEditable(false);
	...
	public int getMouseOperation(){
		return modeler.getMouseOperation();
	}

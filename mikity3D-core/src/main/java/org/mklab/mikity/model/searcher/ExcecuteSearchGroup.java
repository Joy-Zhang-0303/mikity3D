...
//          
//        }
//        this.lastList = parentList;
//        this.lastGroup = group;
        int linkCount = 0;
        while(true) {
          try {
//            if(parentList.isContain(searchLinkData(group, linkCount)) == false) {
            parentList.addItems(searchLinkData(group, linkCount, parentList));
            linkCount++;
  ...
  public static void main(String[] args) {
    ...
    try {
      input = new FileInputStream("../mikity3D-sample/src/main/resources/pendulum/pendulum/pendulum.m3d");
  ...
  private static void sample(Mikity3d root) {
    ...
    int a = 0;
    List<GroupManager> lists = result.get();

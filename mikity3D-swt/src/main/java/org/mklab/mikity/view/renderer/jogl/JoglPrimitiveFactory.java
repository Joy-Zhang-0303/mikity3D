...
public class JoglPrimitiveFactory {
  ...
  public static JoglObject create(PrimitiveModel primitive) {
    ...
    if (primitive instanceof BoxModel) {
      child = new JoglPrimitive(new BoxPrimitive((BoxModel)primitive));
    } else if (primitive instanceof CylinderModel) {
      child = new JoglPrimitive(new CylinderPrimitive((CylinderModel)primitive));
    } else if (primitive instanceof ConeModel) {
      child = new JoglPrimitive(new ConePrimitive((ConeModel)primitive));
    } else if (primitive instanceof SphereModel) {
      child = new JoglPrimitive(new SpherePrimitive((SphereModel)primitive));
    } else if (primitive instanceof TrianglePolygonModel) {
      child = new JoglPrimitive(new TrianglePolygonPrimitive((TrianglePolygonModel)primitive));
    } else if (primitive instanceof QuadPolygonModel) {
      child = new JoglPrimitive(new QuadPolygonPrimitive((QuadPolygonModel)primitive));
    } else {
